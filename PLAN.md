...
| 回复媒体指令 | `MEDIA:/FILE:` directives | adapter final/send hook + `upload_and_send_media` |
| DM 策略 | open/pairing/allowlist/disabled | `policy.py` |
| Group 策略 | open/allowlist/disabled + per-group | `policy.py` |
| Template Card | 发送 + 事件更新 | `template_card.py` + adapter event hook |
| enter_chat 欢迎语 | `replyWelcome` | `monitor.handle_enter_chat_event` + adapter event hook |
| disconnected_event | suppress restart / mark displaced | adapter event hook + connection state |
| 消息处理超时 | `MESSAGE_PROCESS_TIMEOUT_MS` | `run_with_message_timeout` |
| Session recording | message state bookkeeping | `SessionRecorder` |
| ReqId store | chatId -> reqId | `StateManager` + adapter inbound hook |
| 文本防抖聚合 | 500ms debounce batching | session-scoped text batch flush |
| 长流式 keepalive | 4 min keepalive | `StreamTurn.keepalive_handle` |
| Stream rotation | 5 min proactive rotate | finish old stream + new `stream_id` |
| 超长完整内容兜底 | dmContent / proactive fallback | UTF-8 分块 `send_message` 补发 |
| Callback crypto/XML | self-built app callback primitives | `callback.py` |
| Callback HTTP inbound | self-built app HTTP callback | optional aiohttp listener + Hermes dispatch |
| Callback inbound media | Agent API `media/get` | download MediaId + cache into `media_urls` |
| Callback proactive reply | Agent API `message/send` | access-token cache + retry on token expiry |
| Token Lock | — | `acquire_scoped_lock` |
| Cron Delivery | — | `standalone_sender_fn` |
| 消息去重 | 有 | `MessageDeduplicator` |

当前迁移状态和验证记录见 [docs/migration-status.md](./docs/migration-status.md)。
...
XWECOM_CALLBACK_ENABLED=true                           # optional self-built app mode
XWECOM_CALLBACK_PORT=8645                              # optional
XWECOM_CALLBACK_PATH=/wecom/callback                   # optional
XWECOM_CORP_ID=wwxxxxxxxx                              # callback receive id
XWECOM_CORP_SECRET=corp_secret                         # callback replies
XWECOM_AGENT_ID=1000002                                # callback replies
XWECOM_CALLBACK_TOKEN=token                            # callback verification
XWECOM_ENCODING_AES_KEY=43_char_key                    # callback decrypt
```
